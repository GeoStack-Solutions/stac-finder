...
import {normalizeCollection} from "../handleCollections/migrator.js"
...
export function makeHandleSTACObject(deps = {}) {
    ...
    return async function handleSTACObjectImpl(rawSTACObject, Link, parentUrl = null) {
        ...
        // initialize with the raw object as a fallback
        let STACObject = rawSTACObject;

        try {
            // attempt to normalize/migrate the object (Catalog or Collection)
            const result = await normalizeCollection(rawSTACObject, Link);
            STACObject = result.collection; // use clean, migrated version
            
        } catch (migrationErr) {
            // If migration fails, log a warning but continue with the raw object
            // This ensures one bad file doesn't stop the whole crawler
    ...
    try {
        ...
        let collectionToProcess = collection;
            ...
            try {
                ...
                const result = await normalizeCollection(collection, source.url);
                collectionToProcess = result.collection;

            } catch (migrationErr) {
                // if migration fails, we log a warning but skip this specific collection
                // to prevent polluting the DB with invalid data or crashing the crawler.
                logger.warn(`Skipping collection in ${source.url} due to migration error: ${migrationErr.message}`);
                continue; 
            }

            // parse API collection to extract metadata and prepare for DB upsert
            // pass the clean 'collectionToProcess', not the raw 'collection'
            const parsedCollection = parseCollection(collectionToProcess, source.id, new Date());
